...
if (len(sys.argv) < 6):
  print('USAGE: DiffMeansFromStats.py nsample1 average1 stdev1 nsample2 average2 stdev2')
  sys.exit()
n_x = int(sys.argv[1])
...
print(' std.dev ratio from data (s1/s2): {:12.5} '.format(s_ratio))
...
if(MAKEPLOT):
  ...
  plt.title('posterior pdf,cdf for difference in means')
  plt.ylim((0.,1.2))
  plt.grid(True)
plt.show()
